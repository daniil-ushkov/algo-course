...
struct range_tree {
  explicit range_tree(vector<int64_t > &vec)
      : data_(4 * vec.size()),
        set_(4 * vec.size(), INF),
        add_(4 * vec.size(), 0),
        size_(vec.size()) {
 ...
 private:
  ...
  void push_(segment_ node) {
    ...
    if (add_[node.index] != 0) {
      ...
      if (!node.is_leaf()) {
        if (set_[node.left().index] != INF) {
          set_[node.left().index] += add_[node.index];
        } else {
          add_[node.left().index] += add_[node.index];
        }
        if (set_[node.right().index] != INF) {
          set_[node.right().index] += add_[node.index];
        } else {
