...
struct heavy_light {
  explicit heavy_light(tree const &t)
      : t(t),
        ...
        st(t.size_, [](uint64_t a, uint64_t b) { return max(a, b); }), top(t.size_), pos(t.size_) {
    build();
  }
  ...
  uint64_t calc(size_t u, size_t v) {
    ...
    while (top[u] != top[v]) {
      ...
      res = max(res, st.get(pos[top[u]], pos[u] + 1));
      u = t.parent[top[u]];
    }
    if (pos[u] > pos[v]) swap(u, v);
    res = max(res, st.get(pos[u], pos[v] + 1));
    return res;
  }
